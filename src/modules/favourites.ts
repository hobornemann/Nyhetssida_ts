...
export function firstStepsToSaveArticleAsFavourite(el: HTMLButtonElement){
    ...
    if(dataType) addEventListenersToFavouriteButtons(dataType, button);
}

// ----------------------UPDATE FAVOURITE BUTTONS OF RENDERED ARTICLES--------------------------------
export function updateFavouriteButtonsOfRenderedArticles(container:string): Promise<boolean>{ 
        ...
        if(favouriteButtons){
            try{
                ...
                if(favouriteArticles){
                    let renderedArticles: Article[] | undefined = getArticlesFromLocalStorage('renderedArticles-' + container)
                    if(renderedArticles){
...
export function addEventListenersToFavouriteButtons(container: string, clickedButton: HTMLButtonElement): Promise<boolean> {
    return new Promise((resolve, reject) => {
        try{
            const favouriteButtons: HTMLButtonElement[] | null = Array.from(document.querySelectorAll(".favourite-button"))    
            let renderedArticles: Article[] | undefined = getArticlesFromLocalStorage('renderedArticles-' + container)
